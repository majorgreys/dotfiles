...
configs=("fish" "tmux" "vim" "helix" "zsh")
...
if [[ ":$PATH:" != *":$HOME/.config/emacs/bin:"* ]]; then
    ...
    if [[ "$SHELL" == *"fish"* ]]; then
        echo -e "\n# Doom Emacs\nfish_add_path $HOME/.config/emacs/bin" >> "$SHELL_RC"
    else
        echo -e "\n# Doom Emacs\nexport PATH=\"\$HOME/.config/emacs/bin:\$PATH\"" >> "$SHELL_RC"
    fi
    print_success "Doom added to PATH in $SHELL_RC"
fi
