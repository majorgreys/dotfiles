...
;; Provides a small popup at point for completion-at-point sources like
;; agent-shell's /command and @file completions.
(use-package corfu
  ...
  (defun thb/md-table-col-widths (header rows usable)
    ...
    (let* ((ncols (length header))
      ...
      (let ((total (apply #'+ max-w)))
        (if (<= total usable)
            max-w
          ;; Keep short columns, shrink long ones proportionally
      ...
      (save-excursion
        ...
        (let ((inhibit-read-only t)
              (win-width (- (or (when-let ((win (get-buffer-window (current-buffer))))
                                ...
                                (window-body-width))
                             2)))
          ;; Require at least 2 pipe chars to identify a real table row
          (while (re-search-forward "^[ \t]*|[^|\n]+|" nil t)
            (let ((tbl-start (line-beginning-position)))
              ...
              (let* ((tbl-end (point))
                ...
                ;; Only format real tables (header + at least 1 data row, 2+ cols)
